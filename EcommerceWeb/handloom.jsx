...
export function Handloom(props) {
    const [product, setProduct] = useState({ Items4: [{ products: [] }] });
    ...
    function Image(e, id) {
        const ids = product.Items4.find(itm => itm.id === id);
        if (ids) {
    ...
    return (
        <div className="container-fluid-lg">
            ...
            <div id="sareeInfo " className="m-4 mx-3">
                <h5 className="">HANDLOOM SAREES ONLINE</h5>
                <p className="text-justify">
                    Shopping handloom India sarees online is easier and hassle-free than ever! Finest of handloom in silk & cotton with a multitude of hues,
                    design and patterns are offered to you by BharatSthali. BharatSthali has come up with an exclusive Handloom saree brand in India.
                    People often call us etsy of India for premium handloom sarees.
                </p>
            </div>
            <div id="product" className="row align-items-center mx-auto justify-content-center w-100">
                {product.Items4.map((resp, index) => (
                    <div className="col-lg-2 col-sm-12 col-6" key={index}>
                        <div className="card">
                        <img onClick={(e) => Image(e, resp.id)} src={resp.images} style={{ cursor: 'pointer' }} className="img-fluid" alt="" />
